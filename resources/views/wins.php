...
<strong>Total Win:</strong> $<?=htmlspecialchars($totalWin)?></p>

<?php
$matrix = $gridFinal ?? $grid ?? [];
$palette=['#ef4444','#10b981','#3b82f6','#f59e0b','#8b5cf6','#14b8a6','#f43f5e','#22c55e','#eab308','#06b6d4'];
$cell=64; $gap=6; $cols=5; $rows=3;
$svgW=$cols*$cell+($cols-1)*$gap; $svgH=$rows*$cell+($rows-1)*$gap;
...
.layer-boxes,.layer-digits{position:absolute;left:0;top:0;display:grid;
grid-template-columns:repeat(5,64px);gap:6px;}
.layer-svg{position:absolute;left:0;top:0;pointer-events:none;}
.box{width:64px;height:64px;border:1px solid #ddd;border-radius:8px;background:#fff;position:relative;}
.wm{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);
font-weight:900;font-size:32px;color:#111827;opacity:.3;}
.digit{width:64px;height:64px;display:flex;align-items:center;justify-content:center;
